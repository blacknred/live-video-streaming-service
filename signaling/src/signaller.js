...
module.exports = function onConnection(socket) {
            ...
            try {
                for (let user in listOfUsers[socket.userid].connectedWith) {
                    try {
                        ...
                        debug('extra-data-updated.connectedWith', e);
                    }
                }
            } catch (e) {
                debug('extra-data-updated.connectedWith', e);
            ...
            if (roomid && listOfRooms[roomid]) {
                ...
                listOfRooms[roomid].participants.forEach((pid) => {
                    try {
                        ...
                        debug('extra-data-updated.participants', e);
                    }
                });
            }
    ...
    function joinARoom(message) {
        try {
            ...
            // const roomid = message.remoteUserId;
            const roomid = socket.admininfo.sessionid;
...
function appendUser(socket, _params) {
    try {
        let extra = _params.extra;

...
function appendUser(socket, _params) {
    try {
        ...
        console.log('appendUser', e.message);
    }
        ...
        if (adminSocket) {
            ...
            config.enableAdmin === true && Object.keys(listOfUsers).forEach((userid) => {
                try {
                    ...
                    console.log('admin.user-looper', e);
                }
            });
...
module.exports = function onConnection(socket) {
    ...
    if (enableScalableBroadcast) {
        try {
            ...
            ScalableBroadcast._ = Scalability(socket, params.maxRelayLimitPerUser);
        } catch (e) {
            console.log('ScalableBroadcast', e);
        }
    }
